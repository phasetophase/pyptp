...
logger.info("LV Network: %s nodes, %s cables", len(lv_network.nodes), len(lv_network.cables))

# Load medium-voltage network (Vision/VNF format)
mv_network = NetworkMV.from_file("path/to/network.vnf")
logger.info("MV Network: %s nodes, %s lines", len(mv_network.nodes), len(mv_network.lines))

# Access network elements
for node in list(mv_network.nodes.values())[:5]:
    logger.info("Node: %s at %skV", node.general.name, node.general.unom)
